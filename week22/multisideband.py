...
A=np.array([-0.5,1.5,1,0])#-0.4264,1.2792,-0.4264,1.2792])#-1.414,1.414*3,-1,3])#
omega=omegak*A
...
#c = np.polynomial.Polynomial([-1, 32*eta**2 + 32, -48*eta**2])
...
detunings=nu*np.array([1,1,2,2])-delta*(np.array([2,6,1,3])+error)
...
C1=np.sqrt(nu*2*10**-2*(1+n))*tensor(identity(2),identity(2),destroy(size))
C2=np.sqrt(nu*2*10**-2*(1+n))*tensor(identity(2),identity(2),create(size))

sol=mesolve(H,psi,tlist,[],[sy*x,sy*p],args=args,options=options)
#sol=mcsolve(H,psi,tlist,[C1,C2],[sy*x,sy*p],args=args,ntraj=1,options=options)
    ...
    for tone in [2]:
        for n in loop:
            ...
            if sideband == 2:
                ...
                if tone ==2:
                    omega=omega0*np.array([-1.414,1.414*3,-1,3])#-0.5,1.5,1,0])#-0.4264,1.2792,-0.4264,1.2792])
                    #c = np.polynomial.Polynomial([-1, 8*eta**2 + 8, -6*eta**2])
                    c = np.polynomial.Polynomial([-1, 32*eta**2 + 32, -48*eta**2])
                    ...
                    detunings=nu*np.array([1,1,2,2])-delta*(np.array([4,12,1,3])+s_error)
        ...
        for a_error in loop:
            ...
            if sideband == 2:
                ...
                if tone ==2:
                    ...
                    p =  np.polynomial.Polynomial([-1, 32*eta**2 + 32, -48*eta**2])
                    omegak=0.5*omega0*eta*np.exp(0.5*eta**2)
...
num=0
options=qutip.solver.Options(atol=1e-12,rtol=1e-12,nsteps=1e6,store_states=True)
loop1=np.linspace(0.01,0.5,50)
loop2=np.linspace(0,20,81)
sideband=1
tone=1
rwa=1
s_error=0
...
F=np.reshape(np.array(F),[4,50,81])
for i in range(2):
    for j in range(2):
        plt.figure()
        plt.imshow(np.flip(a[2*i+j],axis=0),cmap='jet',vmin=0.6,vmax=1)
        plt.colorbar()

        plt.title('%s sideband %s tone'%(i+1,j+1))
        #plt.xticks([0,10,20,30,40,50,61],[-0.3,-0.2,-0.1,0,0.1,0.2,0.3])
        #plt.yticks([0,10,20,30,40,50],[0.5,0.4,0.3,0.2,0.1,0])
        plt.xlabel('Thermal Occupation')
