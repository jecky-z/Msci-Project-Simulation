...
def H_tilde(t): # not in Lamb-Dicke Regime
    phase=np.exp(-1j*detuning*t)
    a=lowering(d_harmonics)*np.exp(-1j*miu*t)
    a_daggar=raising(d_harmonics)*np.exp(1j*miu*t)
    z=ita*(a+a_daggar)
    H=0.5*hbar*omega0*(phase*np.kron(sigma_plus(),np.exp(1j*z))+phase.T*np.kron(sigma_minus(),np.exp(-1j*z)))
    return H

def dxdt(t,state):
    n=np.kron(unit(2),raising(4)).dot(psi).dot(np.kron(unit(2),raising(4)).dot(psi).T)
    dxdt=-1j*np.matmul(H_tilde(t),state)/hbar
    return dxdt
...
#%%
# Sideband Scan
d_harmonics=5 # number of motional states
miu=1000*2*np.pi
omega0=miu/25
detuning=miu*1
ita=0.1
...
def dxdt(t,state):
    ...
    for i in range(d_harmonics):
        for j in range(d_harmonics):
            if i != j:
                dxdt=dxdt-1j*np.matmul(H_sideband(t,i,j),state)/hbar
    return dxdt
...
for i in range(n_scan):
    ...
    p=y*np.conj(y)
    for j in range(d_harmonics):
        c[j,i]=c[j,i]+max(p[1,j])-min(p[1,j])


for k in range(d_harmonics):
    plt.plot(np.arange(n_scan)*0.1-5,c[k,:],label='|%s,%s>'%(1,k))
